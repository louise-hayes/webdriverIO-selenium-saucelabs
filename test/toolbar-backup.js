...
const LoginPage = require('../lib/Page');

describe('Log in to host site and Check Toolbar rendered', () => {
    ...
    it('Log In Get Host Site Title', async () => {
        ...
        let username = await browser.$('[name="username"]')
        username.setValue(process.env.USERNAME)
        let password = await browser.$('[name="password"]')
        password.setValue(process.env.PASSWORD)
        let loginBtn = await browser.$('.login-button')
        loginBtn.click().then( log => {
